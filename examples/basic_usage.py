...
from jump_diffusion import JumpDiffusionEstimator, JumpDiffusionSimulator
...
def main():
    ...
    return_pct = (path[-1] / path[0] - 1) * 100
    print(f"Final value: {path[-1]:.2f} (return: {return_pct:.1f}%)")
    ...
    # Error analysis
    errors = [
        (estimated_vals[i] - true_vals[i]) / true_vals[i] * 100
