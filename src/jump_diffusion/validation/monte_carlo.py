...
class ValidationExperiment:
    ...
    def run_experiment(self, n_simulations: int = 10, T: float = 1.0,
                      n_steps: int = 252, x0: float = 100.0,
                      seed_base: int = 42) -> pd.DataFrame:
        """
            ...
            try:
                ...
                if est_results['convergence']:
                    ...
                    for param, value in est_results['parameters'].items():
                        ...
                        row[f'{param}_rel_error'] = (value - self.true_params[param]) / self.true_params[param] * 100

                    results.append(row)
                    successful_runs += 1
                    print(f"✓ Experiment {i+1}/{n_simulations} completed successfully")
                else:
                    print(f"✗ Experiment {i+1}/{n_simulations} failed to converge")

            except Exception as e:
                print(f"✗ Experiment {i+1}/{n_simulations} failed with error: {str(e)}")

        print(f"\nCompleted: {successful_runs}/{n_simulations} successful experiments")
    ...
    def analyze_results(self) -> Dict[str, Any]:
        ...
        for param in param_names:
            ...
            stats = {
                ...
                'coverage_95': np.mean(np.abs(rel_errors) <= 5)  # Within 5% of true value
            }
    ...
    def plot_results(self, figsize: tuple = (15, 10)):
        ...
        for i, (param, label) in enumerate(zip(param_names, param_labels)):
            ...
            ax.scatter(np.full_like(estimated_vals, true_val), estimated_vals,
                      alpha=0.6, s=50)

            # Perfect estimation line
            ax.axline((true_val, true_val), slope=1, color='red',
                     linestyle='--', alpha=0.8, label='Perfect Estimation')

            # Confidence bands (±10% and ±20%)
            ax.axhspan(true_val * 0.9, true_val * 1.1, alpha=0.2, color='green',
                      label='±10% band')
            ax.axhspan(true_val * 0.8, true_val * 1.2, alpha=0.1, color='yellow',
                      label='±20% band')
        ...
        ax = axes[5]
        param_biases = [np.mean(self.results[f'{param}_rel_error']) for param in param_names]
        param_rmses = [np.sqrt(np.mean(self.results[f'{param}_error']**2)) / self.true_params[param] * 100
                      for param in param_names]

        x = np.arange(len(param_names))
        width = 0.35

        ax.bar(x - width/2, np.abs(param_biases), width, label='|Bias| (%)', alpha=0.7)
        ax.bar(x + width/2, param_rmses, width, label='Rel. RMSE (%)', alpha=0.7)
